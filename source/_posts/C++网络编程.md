...
网络编程可以先理解成“两个程序通过操作系统收发字节”。Socket可以理解为操作系统给程序开的网络门口。
...
先掌握 IP、端口、TCP/UDP，再看 socket 函数。实际写代码时要避免只背 API 名字，要把客户端和服务端的调用顺序画出来。
...
**套接字（Socket）**可以理解为网络通信的端点，是操作系统提供的一种抽象接口，用于实现不同主机/进程间的网络数据传输。C++ 网络编程基于 **Berkeley Socket API**（跨平台，Windows 下为 Winsock，Linux/macOS 下为 BSD Socket），核心是通过 Socket 接口操作网络协议栈（TCP/IP）。
...
**IP 地址**可以理解为网络中主机的唯一标识，用于定位网络中的设备。
...
**端口**可以理解为主机上进程的标识（16 位无符号整数，范围 0~65535），用于区分同一主机上的不同网络应用。
...
## 三、再往下：网络协议与 Socket 通信

网络数据传输基于 **TCP/IP 协议栈**，Socket 通信主要涉及 **传输层（TCP/UDP）** 和 **网络层（IP）**。

### 1. 核心协议

- **IP 协议**（网络层）：主要承担数据包的路由和寻址（源/目的 IP）这件事，你可以把它看成这一层的专职角色。
...
## 五、把它落到代码里：TCP 通信使用时要抓住的点

TCP（Transmission Control Protocol）可以理解为**面向连接的、可靠的、基于字节流的传输层协议**，核心使用时要抓住的点：
...
## 一、先理解：核心 Socket 函数详解

以下函数基于 **Berkeley Socket API**（Linux/macOS），Windows 平台需通过 **Winsock** 适配（见后文“Windows 平台差异”）。

### 1. `socket()`：创建套接字

当我们需要后续操作时，**作用**：创建一个通信端点（套接字），返回套接字描述符（Linux）或句柄（Windows），就会派上用场。
...
当我们需要与该客户端通信），原服务器套接字继续监听时，**作用**：从已完成连接队列中取出一个客户端连接，返回**新的套接字**（就会派上用场。
...
- 当我们需要后续 `send/recv`）时，**成功**：返回**新的客户端套接字描述符/句柄**（就会派上用场。
...
- **非阻塞模式**：若套接字设为非阻塞，`connect()` 会立即返回 `-1`，需通过 `select/poll` 检查连接可以理解为否完成。
...
3. **跨平台兼容**：
    - Linux 需一般会包含头文件 `<sys/socket.h>`、`<netinet/in.h>`、`<arpa/inet.h>`，这些部分要放在同一个场景里一起理解。

    - Windows 需一般会包含`<winsock2.h>`，链接 `ws2_32.lib`，并优先调用 `WSAStartup`，这些部分要放在同一个场景里一起理解。
...
Socket 通信的核心可以理解为**“创建-绑定-监听-连接-通信-关闭”** 流程，其中 `socket/bind/listen/accept/connect` 是控制连接的关键函数，`send/recv` 是数据传输的核心。Windows 平台需通过 Winsock 初始化，函数与 Linux 高度相似但需注意句柄类型、错误处理和库的链接。掌握这些函数细节，可实现跨平台的客户端/服务器通信（如聊天程序、文件传输工具）。
...
2. **线程同步**可以理解为多线程编程的核心挑战，需合理使用锁机制。
