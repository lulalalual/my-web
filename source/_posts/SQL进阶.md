...
SQL 进阶不是写更花的语句，而是知道一条 SQL 为什么慢、为什么会锁住、为什么数据库崩了还能恢复。

## 进阶先说人话

你可以把数据库想成一个仓库。基础 SQL 是会下单，进阶 SQL 是知道仓库怎么找货、怎么排队、怎么记录账本、出错后怎么恢复。
...
- type：访问方式，越接近 const、ref 越好，ALL 通常表示全表扫描。
- key：实际用到的索引。
- rows：预计扫描行数。
- Extra：是否 Using filesort、Using temporary。

EXPLAIN 不是答案，但能告诉你问题大概在哪里。

## B+ 树索引为什么常用

InnoDB 常用 B+ 树索引。它的特点是树高低，范围查询方便，叶子节点之间有顺序链表。

人话解释：B+ 树像一本分层目录，先翻大目录，再翻小目录，最后找到数据所在页。比从第一页翻到最后一页快得多。

## 聚簇索引和二级索引

InnoDB 的主键索引通常是聚簇索引，叶子节点直接存整行数据。

普通索引是二级索引，叶子节点通常存主键值。通过普通索引找到主键后，可能还要回到主键索引查整行，这叫回表。

~~~text
二级索引 -> 找到主键 -> 回到主键索引 -> 找到整行
~~~

如果查询字段都在索引里，就不用回表，这叫覆盖索引。
...
这个索引适合按 user_id 查，并按 created_at 排序。最左前缀可以理解成：目录是先按 user_id 排，再在同一个 user_id 里按 created_at 排。如果你跳过 user_id 直接查 created_at，目录顺序就用不上。

## 索引不是越多越好

索引会加快查询，但会增加写入成本。因为插入、更新、删除数据时，索引也要维护。

适合建索引的字段：

- 经常出现在 WHERE、JOIN、ORDER BY、GROUP BY 中。
- 区分度较高。
- 查询频率高于写入维护成本。

不适合乱建索引的字段：性别、状态这种区分度很低的字段，除非和其他字段组合使用。
...
1. 用慢查询日志找到 SQL。
2. 用 EXPLAIN 看执行计划。
3. 看是否全表扫描、回表太多、排序临时表太重。
4. 调整索引或改写 SQL。
5. 再压测确认效果。

不要只凭感觉加索引。加错索引可能没效果，还会拖慢写入。

## 事务：一组操作要么都成，要么都不成

事务解决的是完整性问题。比如转账，扣钱和加钱必须一起成功。
...
隔离级别是在并发性能和数据一致性之间做取舍。

- 读未提交：可能读到别人没提交的数据。
- 读已提交：只能读到已提交数据，但同一事务内两次读可能不同。
- 可重复读：同一事务内多次读结果一致，MySQL 默认级别。
- 串行化：最严格，但并发性能最差。
...
锁不是越少越好，也不是越多越好。该保护的数据必须保护，但锁范围太大会拖慢并发。
...
- undo log：支持回滚和 MVCC。
- binlog：记录逻辑变更，常用于主从复制和恢复。

人话理解：redo 负责“崩了也能重做”，undo 负责“错了可以撤销”，binlog 负责“把做过的事记成流水账”。
...
如果问索引为什么快，可以说：索引通过有序结构减少扫描范围，B+ 树让查找路径变短，叶子节点有序也适合范围查询。但索引会占空间，并增加写入维护成本。

如果问 MVCC 和锁的关系，可以说：MVCC 主要优化读写并发，让普通快照读不必阻塞写；但当前读、更新、删除仍然需要锁来保证正确性。

## 总结

SQL 进阶的核心是“先看执行计划，再谈优化”。索引、事务、锁和日志都不是孤立知识点，它们共同保证数据库又快又稳。
...
> 说人话：这一节讲的是“（1）避免全表扫描”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“二、存储过程（Stored Procedure）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

**存储过程**是**预编译的 SQL 语句集合**，存储在数据库中，可通过名称调用，支持参数传递、流程控制（分支、循环）和事务管理，用于封装复杂业务逻辑。

### 1. 存储过程的作用

> 说人话：这一节讲的是“1. 存储过程的作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 存储过程的创建与管理”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）示例：带参数的存储过程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（4）调用存储过程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（5）修改与删除存储过程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 存储过程中的流程控制”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 存储过程的优缺点”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“三、触发器（Trigger）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

**触发器**是与表关联的**特殊存储过程**，当表发生指定事件（`INSERT`/`UPDATE`/`DELETE`）时**自动执行**，用于实现数据校验、审计日志、级联操作等。

### 1. 触发器的核心要素

> 说人话：这一节讲的是“1. 触发器的核心要素”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 触发器的作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 触发器的创建与管理”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）NEW 与 OLD 关键字”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）示例1：数据校验触发器（BEFORE INSERT）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（5）删除触发器”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 触发器的注意事项”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
