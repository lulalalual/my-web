...
MySQL 八股最怕背成碎片。更好的方式是跟着一条 SQL 走完整流程：它怎么进来，怎么找数据，怎么保证并发正确，崩溃后怎么恢复。

## MySQL 先说人话

MySQL 要同时解决三件事：查得快，改得稳，并发不乱。
...
~~~text
客户端发送 SQL -> 连接器校验身份 -> 解析器检查语法 -> 优化器选择方案 -> 执行器访问存储引擎 -> 返回结果
~~~

优化器会决定用哪个索引、表怎么连接、扫描多少数据。执行器负责真正去存储引擎拿数据。

## InnoDB 为什么重要

InnoDB 是 MySQL 常用存储引擎，支持事务、行锁、MVCC、崩溃恢复。

如果面试问 MyISAM 和 InnoDB，可以抓住重点：InnoDB 支持事务和行级锁，更适合高并发业务；MyISAM 不支持事务，更多是历史场景。

## 索引是什么

索引就是数据库里的目录。没有目录就只能一行行扫，有目录就能快速定位。

~~~sql
CREATE INDEX idx_user_name ON user_profile(name);
~~~

索引不是越多越好。它加快查询，但会拖慢写入，因为每次写数据还要维护索引结构。

## B+ 树为什么适合数据库

B+ 树层数低，磁盘 IO 少，叶子节点有序，适合范围查询。

人话理解：数据库不喜欢频繁随机读磁盘。B+ 树通过分层目录减少磁盘访问次数，同时把叶子节点串起来，查范围时可以顺着往后走。

## 聚簇索引和回表

InnoDB 主键索引的叶子节点存整行数据，叫聚簇索引。

普通索引叶子节点通常存主键值。用普通索引查到主键后，如果还要整行数据，就要回表。

~~~text
普通索引 -> 主键值 -> 主键索引 -> 整行数据
~~~

覆盖索引可以避免回表，因为查询需要的字段都在索引里。
...
- 脏读：读到别人还没提交的数据。
- 不可重复读：同一事务两次读同一行，结果变了。
- 幻读：同一事务两次范围查询，行数变了。

MySQL InnoDB 默认可重复读，并通过 MVCC 和锁解决大部分问题。

## MVCC：给读操作一个快照

MVCC 可以理解成“读历史快照”。读操作不一定要等写操作结束，而是根据版本链和 Read View 判断当前事务能看到哪个版本。

它的价值是提高读写并发：别人正在改数据时，你仍然能读到一个一致的旧版本。
...
锁范围越小，并发越好；锁时间越短，系统越稳。

## redo log、undo log、binlog

redo log 保证崩溃恢复。事务提交后，即使数据页还没刷盘，也能通过 redo 恢复。

undo log 支持回滚，也支持 MVCC 读取旧版本。

binlog 记录逻辑变更，常用于主从复制和数据恢复。
...
主库写 binlog，从库读取并重放。它解决读扩展和备份问题，但会有主从延迟。

如果业务要求强一致，读写分离时要注意刚写完马上读可能读到旧数据。

## 常见面试问法

问“为什么索引会失效”，可以说：对索引列做函数运算、隐式类型转换、like 前缀通配、联合索引不满足最左前缀，都可能导致索引用不上。

问“可重复读怎么解决幻读”，可以说：普通快照读靠 MVCC 保持一致视图，当前读和范围更新则可能使用 next-key lock 控制插入。

问“redo 和 binlog 区别”，可以说：redo 是 InnoDB 物理日志，用于崩溃恢复；binlog 是 Server 层逻辑日志，用于复制和恢复。

## 总结

MySQL 的核心不是背术语，而是理解它怎样同时做到快、稳、并发安全。索引、事务、锁、MVCC、日志都围绕这个目标服务。
...
> 说人话：这一节讲的是“步骤1：客户端连接与连接器验证”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“步骤3：分析器（Analyzer）—— 词法+语法分析”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“步骤5：执行器（Executor）—— 调用存储引擎执行计划”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 数据库操作命令”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 表操作命令”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

#### （1）创建表（核心语法）

> 说人话：这一节讲的是“（1）创建表（核心语法）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）查看表结构”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）修改表结构（ALTER TABLE）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（4）删除表”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 数据操作命令（CRUD）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）更新数据（UPDATE）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（4）删除数据（DELETE）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“5. 用户与权限命令（管理员用）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“6. 其他常用命令”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“关键说明”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“组成（4个核心参数）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“可见性判断规则（核心！）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. RC与RR级别下ReadView的生成时机”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 解决脏读”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 解决不可重复读（RR级别）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

RR级别第一次查询生成ReadView后复用，后续查询看到的仍是第一次查询时的版本链快照，即使其他事务提交了修改（`trx_id`可能在`m_ids`外），也因ReadView未更新而**不可见**。

### 3. 减少幻读（RR级别）

> 说人话：这一节讲的是“3. 减少幻读（RR级别）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 实现方式与适用场景”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 关键区别”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 工作流程（核心作用）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 特点”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）查询`information_schema`系统表”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“五、总结与高频面试问题”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“分析工具”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“show processlist”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“performance_schema”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 其他工具”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（4）批量操作代替循环单条操作”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）范式与反范式平衡”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）拆分大表”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“三、查看EXPLAIN：分析执行计划”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

### 1. EXPLAIN作用

> 说人话：这一节讲的是“1. EXPLAIN作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. EXPLAIN输出字段详解（核心！）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. EXPLAIN示例分析”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“一、主从复制（Master-Slave Replication）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

### 1. 定义与核心作用

> 说人话：这一节讲的是“1. 定义与核心作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 主从复制原理（核心流程）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

基于**二进制日志（Binlog）** 实现，核心涉及**3个线程**和**2种日志**（Binlog、Relay Log）。

#### （1）核心组件

> 说人话：这一节讲的是“（1）核心组件”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）复制流程（面试重点）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（1）延迟表现”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

从库数据落后于主库（如主库写入后，从库需几秒甚至几分钟才能读到）。

#### （2）延迟原因

> 说人话：这一节讲的是“（2）延迟原因”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“二、分库分表（Sharding）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

### 1. 定义与核心原因

> 说人话：这一节讲的是“1. 定义与核心原因”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）垂直分表（按字段拆分）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（3）水平分库（按数据行拆分到多库）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（4）水平分表（按数据行拆分到多表）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 分库分表实现方式”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

#### （1）客户端分片（应用层分片）

> 说人话：这一节讲的是“（1）客户端分片（应用层分片）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 分库分表后的挑战与解决方案”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“一、存储引擎的定义与作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

**存储引擎**是MySQL中负责**数据存储、检索、索引管理、事务处理**的底层组件，决定了表数据的物理存储方式和功能特性。MySQL采用**插件式架构**，支持多种存储引擎，用户可根据业务需求选择（如事务、锁粒度、性能）。

## 二、常见存储引擎及核心特性

> 说人话：这一节讲的是“二、常见存储引擎及核心特性”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“**核心定位**”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 其他存储引擎（了解）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“三、InnoDB vs MyISAM（面试高频对比）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“四、存储引擎的选择依据”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“五、存储引擎的管理命令”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

### 1. 查看支持的存储引擎

> 说人话：这一节讲的是“1. 查看支持的存储引擎”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 创建表时指定引擎”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 修改表引擎”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 配置默认引擎（my.cnf）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 核心作用”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 三种格式对比”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 核心参数”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 工作原理”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）写入流程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 存储结构”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 配置参数”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 分析工具”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“4. 注意事项”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 工作流程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“十、总结与高频面试问题”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“步骤3：执行器调用InnoDB引擎执行更新”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. ROW格式（记录行变更）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. MIXED格式（自动选择ROW）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 问题背景”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

InnoDB的Redo Log（物理日志，记录页修改）与Binlog（逻辑日志，记录数据变更）需保证**一致性**：事务提交时，要么两者都写入，要么都不写入（避免主从数据不一致或崩溃后恢复失败）。

### 2. 二阶段提交流程（2PC）

> 说人话：这一节讲的是“2. 二阶段提交流程（2PC）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. Crash恢复逻辑”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 核心参数”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 管理命令”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 核心原理：从“全表扫描”到“有序查找””。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. **模糊查询以%开头**”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“5. **!=或<>操作符**”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 核心结论”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“1. 定义与存储结构”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 核心区别”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 示例：InnoDB查询流程”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 匹配规则（以`(a,b,c)`为例）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“2. 工作流程（对比无ICP）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。

以联合索引`(a,b)`，查询`SELECT * FROM t WHERE a=1 AND b>2;`为例：

#### （1）无ICP（5.6前）

> 说人话：这一节讲的是“（1）无ICP（5.6前）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“（2）有ICP（5.6+）”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“3. 适用条件”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
...
> 说人话：这一节讲的是“九、总结与高频面试问题”。先别急着背定义，先抓住它解决的问题、代码里怎么写、边界在哪里。
