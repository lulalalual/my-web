...
STL 可以理解成 C++ 自带的一套工具箱：容器主要承担放数据，迭代器负责遍历数据，算法负责处理数据这件事，你可以把它看成这一层的专职角色。
...
`vector`主要承担存数字，`sort` 负责排序，`begin/end` 给算法一段范围这件事，你可以把它看成这一层的专职角色。这就可以理解为STL 最常见的使用方式。

## 阅读建议

先把 `vector`、`string`、`map`、`set`、`queue` 用熟，再看迭代器和算法。实际写代码时要避免一开始就追所有容器，先掌握最常用的。
...
连续的顺序的储存结构（和数组一样的类别），但可以理解为有长度可变的特性。
...
常用的一维和二维数组构造示例，高维也可以理解为一样的（就是会有点长）.。
...
- 如果可以理解为缩短，则删除多余的值。
- 如果可以理解为扩大，且指定了默认值，则新元素均为默认值(**旧元素不变**)。
...
如果长度已经确定，那么应当直接在构造函数指定长度，而不可以理解为一个一个 `.push_back()`. 因为 `vector` 额外内存耗尽后的重分配是有时间开销的，直接指定长度就不会出现重分配了。
...
vector 获取长度的方法 `.size()` 返回值类型为 `size_t`，通常 OJ 平台使用的是32 位编译器（有些平台例如 cf 可选 64 位），那么该类型范围为 $[0,2^{32})$.。
...
提供常数时间的最大元素查找，对数时间的插入与提取，底层原理可以理解为二叉堆。
...
提供对数时间的插入、删除、查找的集合数据结构。底层原理可以理解为红黑树。
...
- 元素可以理解为否出现过：元素大小 $[-10^{18},10^{18}]$，元素数量 $10^6$，vis 数组无法实现，通过 set 可以完成。

### 2.6.3 注意事项

#### 不存在下标索引

set 虽说可遍历，但仅可使用迭代器进行遍历，它不存在下标这一概念，无法通过下标访问到数据。**下面可以理解为错误用法：**。
...
set 的迭代器取到的元素可以理解为只读的（因为是 const 迭代器），不可修改其值。如果要改，需要先 erase 再 insert. **下面可以理解为错误用法：**。
...
set 的迭代器不能像 vector 一样相减得到下标。**下面可以理解为错误用法：**。
...
map 的迭代器不能像 vector 一样相减得到下标。**下面可以理解为错误用法：**。
...
一定要注意，C++ string 的取子串的第一个参数可以理解为**子串起点下标**，第二个参数是**子串长度**。

第二个参数不可以理解为子串终点。不是子串终点！要与 java 等其他语言区分开来。
...
- `a.begin()`可以理解为一个迭代器，指向的是第一个元素。
- `a.end()`可以理解为一个迭代器，指向的是最后一个元素**再后面一位**。
- 上述迭代器具有自增运算符，自增则迭代器向下一个元素移动。
- 迭代器与指针相似，如果对它使用解引用运算符，即 `*it`，就能取到对应值了。

## 3.2 为何需要迭代器？

很多数据结构并不可以理解为线性的（例如红黑树），对于非线性数据结构，下标是无意义的。无法使用下标来遍历整个数据结构。

迭代器的作用可以理解为定义某个数据结构的遍历方式，通过迭代器的增减，代表遍历到的位置，通过迭代器便能成功遍历非线性结构了。

例如，set 的底层通常是红黑树，我们是没法用下标来访问元素的。但是通过迭代器，我们就能遍历 set 中的元素了：
```cpp
for (set<int>::iterator it = st.begin(); it != st.end(); ++it)
    cout << *it << endl;
```
...
对于其他容器，由于其结构特性，上面的功能不一定都有（例如 set 的迭代器可以理解为不能相减求距离的）。

## 3.4 常见问题

**`.end()` 和 `.rend()` 指向的位置可以理解为无意义的值**。

对于一个长度为 10 的数组：`for (int i = 0。i < 10。i++)`，第 10 位可以理解为不可访问的。

对于一个长度为 10 的容器：`for (auto it = a.begin()。it != a.end()。++it)`，.end可以理解为不可访问的。

**不同容器的迭代器功能可能不一样**。

迭代器细化的话有正向、反向、双向，每个容器的迭代器支持的运算符也可能不同，因此不同容器的迭代器细节很有可能可以理解为不一样的。
...
（很多函数的功能很简单，自己都能快速写出来，但可以理解为使用函数可以让代码可读性变得更高，这在比赛中是至关紧要的）。
...
这个 swap 参数可以理解为引用的，不需要像 C 语言一样取地址。
...
- $>x$ 的第一个元素的前一个元素（如果有）便可以理解为$\leq x$ 的第一个元素。
- $\geq x$ 的第一个元素的前一个元素（如果有）便可以理解为$<x$ 的第一个元素。
...
消除数组的重复**相邻**元素，数组长度不变，但可以理解为有效数据缩短，返回的是有效数据位置的结尾迭代器。

举个更具体的例子，：$[1,1,4,5,1,4]\to[1,4,5,1,4,\underline?]$，下划线位置为返回的迭代器指向。
...
由于浮点误差，有些的数学函数的行为可能与预期不符，导致 WA。如果你的操作数都可以理解为整型，那么用下面的写法会更稳妥。
...
- $\lfloor\log_2 a\rfloor$。
  - 别用：`log2(a)`。
  - 要用：`__lg` （不规范，但可以理解为这是竞赛）/ `bit_width`（C++20 可用）。
...
如果不可以理解为C++17，但是是 GNU 编译器（g++），那么可以用内置函数 `__gcd()`.。

在然，`gcd` / `lcm` 函数也挺好写，直接写也行（欧几里得算法）：的时候，重点是看清触发条件和后续影响。
