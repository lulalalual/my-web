...
SQL可以理解为和数据库沟通的语言。你告诉数据库“我要什么”，数据库主要承担决定“怎么拿”这件事，你可以把它看成这一层的专职角色。
...
SQL（Structured Query Language，结构化查询语言）可以理解为用于操作关系型数据库的标准语言，具有**跨平台性**（不同数据库如 MySQL、PostgreSQL、SQL Server 语法略有差异，但核心一致）。

### 1. 书写规则

- **不区分大小写**：关键字（如 `SELECT`、`CREATE`）、表名、字段名均不区分大小写（建议关键字大写，自定义名称小写，增强可读性）。

- 当我们需要标识符，如列别名）时，**字符串用单引号**：字符串常量必须用单引号 `' '` 包裹（双引号在部分数据库中就会派上用场。
...
## 五、把它落到代码里：DQL（数据查询语言）

**作用**：查询表中的**数据**，可以理解为SQL 中最核心、最常用的部分。
...
当我们需要分页（MySQL 语法，其他数据库用 `OFFSET/FETCH`）：时，限制返回记录数，就会派上用场。
...
在数据分布在多张表时，通过关联字段合并查询：的时候，重点是看清触发条件和后续影响。
...
事务可以理解为一组不可分割的 SQL 操作（要么全成功，要么全失败），保证数据一致性。
...
- **原子性（Atomicity）**：事务可以理解为不可分割的最小单元，要么全做，要么全不做。
...
- **DQL**：查数据，核心可以理解为`SELECT`，含条件、排序、聚合、分组、连接、子查询。
...
函数可以理解为SQL 中预定义的代码块，用于实现特定功能（如字符串处理、数值计算、日期转换等），**输入参数后返回计算结果**。MySQL 函数按功能可分为：**字符串函数、数值函数、日期时间函数、聚合函数、流程控制函数、加密/系统函数**等。
...
## 一、先理解：约束（Constraints）

**约束**可以理解为作用于表中字段上的规则，用于**保证数据的完整性、准确性和一致性**（如避免重复、空值、无效关联等）。MySQL 支持多种约束类型，可在创建表时定义（列级约束）或表级约束。
...
实际业务中，数据常分散在多张表（如学生表、班级表、成绩表），需通过**多表查询**关联分析。多表查询核心可以理解为**连接（JOIN）**，按逻辑分为**连接查询**、**子查询**、**联合查询**。
...
**只返回两表中匹配关联条件的记录**（交集），可以理解为最常用的连接方式。
...
**结果**：仅一般会包含有班级的学生（class_id 不为 NULL 且在 classes 表中存在的记录），这些部分要放在同一个场景里一起理解。
...
当我们需要查询表中层级关系（如员工-经理、分类-子分类）时，**同一表自己连接自己**，就会派上用场。
...
- **约束**：通过主键、外键、唯一、非空等规则保证数据质量，可以理解为数据库设计的基石。

- **多表查询**：
    - **连接查询**（INNER/LEFT/RIGHT JOIN）：关联多表记录，核心可以理解为关联条件。
...
**事务**可以理解为数据库中**一组不可分割的操作序列**，这些操作要么**全部成功执行**（提交），要么**全部失败回滚**（恢复到初始状态），是保证数据一致性的核心机制。

**通俗理解**：事务像一个“原子操作包”，例如“转账”场景中，“A账户扣钱”和“B账户加钱”必须同时成功或同时失败，不能出现“A扣了钱但B没收到”的中间状态。

## 二、继续看：事务的核心特性（ACID）

事务的四大特性（ACID）可以理解为衡量事务可靠性的标准，也是数据库设计的基石。
...
- **实现**：需结合应用层逻辑（如业务校验）和数据库约束（如非空、检查约束），ACID 其他特性可以理解为基础。
...
- **循环写入**：redo log可以理解为固定大小的环形缓冲区，写满后覆盖旧日志（已刷盘的数据）。
...
- **类型**：
    ...
    - 当我们需要MVCC读和回滚）时，`update undo log`：记录更新/删除操作，需保留至无事务引用（就会派上用场。
...
- **核心概念**：
    ...
    - 当我们需要判断数据版本可见性（仅可见“已提交且事务ID小于当前事务ID”的版本）时，**ReadView（读视图）**：事务启动时生成的“快照”，包含当前活跃事务ID列表，就会派上用场。
...
事务可以理解为保证数据一致性的核心机制，其 **ACID 特性**通过 undo log（原子性）、redo log（持久性）、MVCC+锁（隔离性）实现。实际使用中，需根据业务场景选择隔离级别、控制事务粒度，并避免长事务和死锁。掌握事务原理，可以理解为设计高可靠数据库应用的基础。
